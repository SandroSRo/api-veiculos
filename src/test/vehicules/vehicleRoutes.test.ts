...
describe("Tests for route /api/vehicles", () => {
  ...
  it("Should update one vehicule", async () => {
    ...
    const vehicleData = {
      brand: "TestBrand-Update",
      vehicleModel: "TestModel-Update",
      year: 2025,
    };
  ...
  it("Should deleted one vehicule", async () => {
    ...
    const id = data.body[0]._id;
    await request(app).delete(`/api/vehicles/${id}`).expect(200);
  });
  ...
  it("Should failed in deleted one vehicule", async () => {
    ...
    const id = "123456789";
    await request(app).delete(`/api/vehicles/${id}`).expect(500);
  });
